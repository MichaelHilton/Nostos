...
// MARK: - Design system stubs (appended to an included source so Xcode will compile them)
import SwiftUI
...
struct DiamondAccent: View {
    ...
    var body: some View { Rectangle().frame(width: size, height: size).foregroundColor(.nostosGreen) }
}
...
struct ScannerIcon: View { var active = false; var body: some View { Image(systemName: "scanner") } }
struct GalleryIcon: View { var active = false; var body: some View { Image(systemName: "photo.on.rectangle") } }
struct DuplicatesIcon: View { var active = false; var body: some View { Image(systemName: "doc.on.doc") } }
struct VaultIcon: View { var active = false; var body: some View { Image(systemName: "lock.shield") } }
...
struct PageHeaderView<Actions: View>: View {
    ...
    init(title: String, subtitle: String? = nil, @ViewBuilder actions: () -> Actions = { EmptyView() as! Actions }) {
        self.title = title
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading) {
                ...
                if let subtitle = subtitle { Text(subtitle).font(.subheadline) }
            }
            Spacer()
            actions
        }
...
struct WaveLensLogo: View { var body: some View { Text("WL") } }

extension Font {
    static func nostosDisplay(size: CGFloat = 17, weight: Font.Weight = .regular) -> Font {
...
extension Color {
    ...
    static let nostosFg2 = Color.secondary
    static let nostosFg3 = Color.gray
    static let nostosBg = Color.white
    static let nostosAccent = Color.accentColor
    static let nostosSurface2 = Color(white: 0.95)
    static let nostosSidebar = Color(white: 0.98)
    static let nostosSidebarBorder = Color(white: 0.9)
}

// Additional stubs required by Views
extension Color {
...
struct CardView<Content: View>: View {
    ...
    var body: some View {
        VStack { content }
            ...
            .cornerRadius(8)
            .overlay(RoundedRectangle(cornerRadius: 8).stroke(Color.nostosSidebarBorder))
    }
}
    ...
    var body: some View {
        GeometryReader { g in
            ZStack(alignment: .leading) {
                ...
                RoundedRectangle(cornerRadius: 4).fill(Color.nostosAccent).frame(width: max(0, CGFloat(value) * g.size.width), height: 8)
            }
        }.frame(height: 8)
    }
}
